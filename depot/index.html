...
.pnl-badge.up { color: var(--green); } .pnl-badge.down { color: var(--red); } .pnl-badge { color: var(--muted); }
...
.lot-pnl.up { color: var(--green); } .lot-pnl.down { color: var(--red); } .lot-pnl { color: var(--muted); }
.lot-sell-btn {
...
function updateSummary() {
  ...
  el.style.color = Math.abs(pnl) < 0.005 ? 'var(--muted)' : pnl > 0 ? 'var(--green)' : 'var(--red)';
}
...
function renderPositions() {
  ...
  groups.forEach((g, gi) => {
    ...
    if (existing) {
      ...
      pb.className = 'pnl-badge ' + (pnlClass(pnlPct, 'up', 'down'));
      ...
      pe.className = 'meta-val mv-pnl ' + (pnlClass(pnlAbs, 'pos', 'neg'));
    ...
    div.innerHTML = `
      <div class="pos-header" onclick="toggleGroup('${g.symbol}')">
        ...
        <div class="pnl-badge ${pnlClass(pnlPct, 'up', 'down')}">${fmtPct(pnlPct)}</div>
      ...
      <div class="pos-meta">
        ...
        <div><div class="meta-label">G/V</div><div class="meta-val mv-pnl ${pnlClass(pnlAbs, 'pos', 'neg')}">${(pnlAbs >= 0 ? '+' : '') + fmtChf(pnlAbs)}</div></div>
      </div>
  ...
  return [...g.lots].sort((a,b) => new Date(b.boughtAt) - new Date(a.boughtAt)).map(lot => {
    ...
    return `
      <div class="lot-row">
        ...
        <div class="lot-pnl ${pnlClass(pnlAbs, 'up', 'down')}">${fmtPct(pnlPct)}</div>
  ...
  container.innerHTML = groups.map(g => {
    ...
    return `
      <div class="sell-row">
        ...
        <span class="sell-pnl ${pnlClass(pnl, 'up', 'down')}">${fmtPct(pnl)}</span>
